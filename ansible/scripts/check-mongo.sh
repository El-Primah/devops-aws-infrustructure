...
while true; do
    ...
    if [[ "$current_restart_count" -gt "$previous_restart_count" ]]; then
     ...
     if [[ "$current_started_timestamp" -lt "$previous_started_timestamp" ]]; then
         curl -X POST -H 'Content-type: application/json' --data '{"text":"WARNING! Pod mongodb-0 in namespace dfg was restarted, since the working time has decreased. ENV: Automation"}' "$SLACK_WEBHOOK_URL"
